...
const NO_AFFECTED = {
  ...
  draft: { dish: 0, weekly: 0, monthly: 0, news: 0, 'page:home': 0, 'page:takeaway': 0, 'page:about': 0 },
}
...
describe('saveImageAltText — the one direct column write (brief §10)', () => {
  ...
  it('a failed references read after a successful save falls back to the five entity tags — bounded, never global', async () => {
    ...
    expect([...result.cacheTags].sort()).toEqual(['menu', 'monthly', 'news', 'page:about', 'page:home', 'page:takeaway', 'weekly'])
  })
  ...
  it('expires the tags of the LIVE rows the function reports, before the files go (10C-2, brief §20)', async () => {
    ...
    const db = rpcDatabase({
      data: {
        ...
        affected: {
          live: { dish: 1, weekly: 0, monthly: 0, news: 1, 'page:home': 0, 'page:takeaway': 0, 'page:about': 0 },
          draft: { dish: 0, weekly: 1, monthly: 1, news: 0, 'page:home': 0, 'page:takeaway': 0, 'page:about': 0 },
        },
      },
    })
  ...
  it('expires the tags of the LIVE rows the function reports, before the old files go (10C-2, brief §21)', async () => {
    ...
    const db = rpcDatabase({
      data: {
        ...
        storage_path: 'old/original.jpg',
        affected: {
          live: { dish: 2, weekly: 1, monthly: 0, news: 0, 'page:home': 0, 'page:takeaway': 0, 'page:about': 0 },
          draft: { dish: 1, weekly: 0, monthly: 1, news: 0, 'page:home': 0, 'page:takeaway': 0, 'page:about': 0 },
        },
      },
    })
    const { storage } = recordingStorage()
  ...
  it('a draft-only replacement expires nothing', async () => {
    const db = rpcDatabase({
      data: {
        ...
        new_id: NEW_ID,
        storage_path: 'old/original.jpg',
        affected: { live: NO_AFFECTED.live, draft: { dish: 1, weekly: 0, monthly: 0, news: 1, 'page:home': 0, 'page:takeaway': 0, 'page:about': 0 } },
      },
    })
    const { storage } = recordingStorage()
    const { effects, expired } = recordingEffects()
