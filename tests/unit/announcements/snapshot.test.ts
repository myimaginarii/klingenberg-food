...
import { ANNOUNCEMENT_SOURCES } from '@/lib/announcements/ownership'
import {
 ...
 * asserts the same nine keys and the same refusals in SQL, from real JWTs, inside the
 ...
 *
 * **8C-3A added the ninth key**, `source_override_id`, and with it one rule no
 * per-field schema can state: a generated announcement names exactly one override and
 * a manual one names none. `supabase/tests/017_generated_announcement.test.sql`
 * asserts the same pairing in SQL.
 *
...
/** A generated announcement, owned by the override that composed it. */
const OVERRIDE_ID = '11111111-2222-4333-8444-555555555555'

const GENERATED: AnnouncementSnapshot = {
  ...A,
...
describe('the snapshot shape', () => {
  it('is exactly the nine published fields', () => {
  ...
  it('accepts a generated snapshot, with the override that owns it', () => {
    expect(parseAnnouncementSnapshot(GENERATED)).toEqual(GENERATED)
  })

  it('carries the three state fields a restore cannot do without', () => {
    ...
    // without `source_override_id` it would come back owned by nobody, and §7e item 6
    // could no longer say which date it points at.
    expect(A.is_visible).toBe(true)
    expect(ANNOUNCEMENT_SOURCES).toEqual(['manual', 'opening_hours'])
    expect(GENERATED.source_override_id).toBe(OVERRIDE_ID)
  })
})
...
describe('what is not a snapshot', () => {
  ...
  it('refuses a generated snapshot that names no override', () => {
    ...
    expect(
      parseAnnouncementSnapshot({ ...GENERATED, source_override_id: null }),
    ).toBeNull()
  })

  it('refuses a manual snapshot that names one', () => {
    expect(parseAnnouncementSnapshot({ ...A, source_override_id: OVERRIDE_ID })).toBeNull()
  })

  it('refuses an owner that is not a uuid', () => {
    expect(
      parseAnnouncementSnapshot({ ...GENERATED, source_override_id: 'soendag' }),
    ).toBeNull()
  })
...
describe('building a snapshot from the published row', () => {
  it('takes the published values, the visibility and the ownership', () => {
    expect(
      announcementSnapshotFrom({
        values: PUBLISHED,
        isVisible: true,
  ...
  it('writes both halves of the pair from one ownership value', () => {
    // The pair cannot be assembled inconsistently on the way in: there is no argument
    // here that spells "generated, owned by nobody".
    const owned = announcementSnapshotFrom({
      values: PUBLISHED,
      isVisible: true,
      ownership: { kind: 'generated', overrideId: OVERRIDE_ID },
    })

    expect(owned.source).toBe('opening_hours')
    expect(owned.source_override_id).toBe(OVERRIDE_ID)
    expect(parseAnnouncementSnapshot(owned)).toEqual(owned)
  })
