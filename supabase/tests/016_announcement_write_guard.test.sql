 ...
 *
 * `source = 'opening_hours'` names exactly one published override
 * (`announcement_source_owner_check`), so both the legitimate replacement below and
 * the *forged* snapshot need a real one to point at. The forgery pointing at a genuine
 * override is deliberate: it is what makes the attack a fair test of the guard rather
 * than of the shape validator.
 */
...
-- ===========================================================================
--
-- Twelve columns, and they are not a wish list: eleven of them are there only because
...
select set_eq(
  ...
  'authenticated may UPDATE exactly twelve named columns — the table-level grant is gone');
...
/*
 * The ownership pointer — phase 8C-3A, and a trusted lifecycle field for §0l's own
 * reason: `restore_announcement()` writes it back out of `previous` without asking
 * anything else about it, and §7e item 6 decides whether to take a live message down
 * by reading it. A staff member who could set it directly could make any override
 * appear to own any announcement.
 *
 * Refused on its own, and refused together with `source` — because the two are one
 * fact in two columns, and a guard that only watched the pair would miss a write that
 * moved one of them.
 */
select throws_ok(
  format($$ update public.announcement set source_override_id = %L $$, pg_temp.owner_override()),
  '42501', null, 'staff cannot point the announcement at an override directly');

select throws_ok(
  format($$ update public.announcement
               set source = 'opening_hours', source_override_id = %L $$, pg_temp.owner_override()),
  '42501', null, 'and cannot forge the pair together either');

reset role;
select is(pg_temp.row_state(), current_setting('test.before')::jsonb,
  'and not one of the ten refusals moved anything');
...
select throws_ok(
  format($$ update public.announcement set source_override_id = %L $$, pg_temp.owner_override()),
  '42501', null, 'owner cannot point the announcement at an override directly either');
...
--
-- 8C-3A added a ninth key and did not change that reading. The forged snapshot below
-- names a **real, published** override as its owner, so the pairing rule the validator
-- gained is satisfied too: a shape validator still cannot see who wrote the value.
...
select is(
  (select public.replace_announcement('Besked B — erstatningen', 'url', null,
     ...
     pg_temp.version(), pg_temp.owner_override()) ->> 'replaced'),
  'active',
