...
| A3 | ~~Final map asset and its licence~~ — **closed by phase 14B3**, not supplied. The static map, its placeholder, its `public/map/LICENSE.md` provenance record and its build-time guard (`lib/site/map-launch-guard.ts`) are retired; the map is now a Google Maps embed (§7g, §0ap) | **Repository proven**: `lib/site/map-embed.ts`, `components/site/GoogleMap.tsx`, `frame-src` in the CSP. Replaces A3 with A3′ below | — | 2026-09-06 · phase 14B3 |
| A3′ | `GOOGLE_MAPS_EMBED_API_KEY` provisioned in Vercel (Google Maps Platform, Maps Embed API enabled) and restricted by HTTP referrer to the production domain once it exists (§13 item D) | Requires production infrastructure — optional: the embed already renders without it (the keyless form) | The developer, once a Google Cloud project exists | |
