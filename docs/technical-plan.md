...
- **`app/robots.ts`** — allow all; sitemap reference. Nothing is disallowed: `/admin` and `/api` belonged to the retired administration and do not exist in the static export. Allowing the crawl is what lets a search engine fetch and index the six public pages. The pre-launch `noindex, nofollow` in `app/layout.tsx` was removed at launch, and `robots.ts` did not change; the 404 keeps the framework's own `noindex`.
