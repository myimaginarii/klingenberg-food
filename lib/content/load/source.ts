 ...
 *
 * EACH FILE IS PARSED ONCE per process, and every caller is handed the same value.
 * That is not only cheaper — it is what keeps the rendered output identical to the
 * module constants this replaced. The shell and a page both read the opening hours,
 * and React's payload serialiser writes the second occurrence of a value it has
 * already seen as a back-reference; two equal-but-separate objects would be written
 * out twice. The content is build-time constant and nothing mutates it, so one value
 * per file is the honest model as well as the small one.
 *
...
function contentPath(segments: readonly string[]): string {
